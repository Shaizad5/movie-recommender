...
"similar_movies": "\U0001f50e \u0938\u092e\u093e\u0928 \u092b\u093f\u0932\u094d\u092e\u0947\u0902",
...
"trending": "\U0001f4c8 \u091f\u094d\u0930\u0947\u0902\u0921\u093f\u0902\u0917 \u092b\u093f\u0932\u094d\u092e\u0947\u0902",
...
if not st.session_state.logged_in:
    ...
    st.markdown(
        "<div style='font-family:Playfair Display,serif;font-size:1.8rem;color:#f0ece4;margin-bottom:4px'>\U0001f3ac CineScope</div>"
        ...
        for c in range(cols):
            ...
            with colset[c]:
                if poster:
                    ...
                    st.markdown("<div style='background:#1a1a26;border-radius:8px;height:180px;display:flex;align-items:center;justify-content:center;color:#7a7590;font-size:2rem'>\U0001f3ac</div>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown(
        ...
        f"\U0001f516 {len(st.session_state.watchlist)} &nbsp;|&nbsp; ⭐ {len(st.session_state.user_ratings)} &nbsp;|&nbsp; \U0001f4dd {sum(len(v) for v in st.session_state.reviews.values())}</div>",
        unsafe_allow_html=True,
    )
    st.divider()
        ...
        with ptab3:
            if st.session_state.watchlist:
                for w in st.session_state.watchlist:
                    ...
                    with c3:
                        if st.button("Open", key=f"prof_wl_{w['tmdb_id']}"): goto("details", w["tmdb_id"])
    ...
    for i, mood in enumerate(MOODS):
        with cols[i]:
            ...
            st.markdown(
                ...
                f"<div class='mood-desc'>{mood['desc']}</div></div>",
                unsafe_allow_html=True,
            )
...
# ==========================================================
    ...
    if not st.session_state.watchlist:
        st.markdown(
            ...
            f"<div style='font-size:3rem'>\U0001f3ac</div>"
        ...
        if wl_view == T("grid"):
            ...
            for i, w in enumerate(st.session_state.watchlist):
                c1, c2, c3 = st.columns([1, 5, 1])
                with c1:
                    if w.get("poster_url"): st.image(w["poster_url"], width=70)
                    else: st.markdown("<div style='width:70px;height:95px;background:#1a1a26;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#7a7590;font-size:1.5rem'>\U0001f3ac</div>", unsafe_allow_html=True)
                with c2:
                    genre_list = [g.strip() for g in (w.get("genres","")).split(",") if g.strip()]
                    genre_html = "".join(f"<span class='genre-pill'>{g}</span>" for g in genre_list)
                    st.markdown(f"<div class='wl-title'>{w['title']}</div><div style='margin:5px 0'>{genre_html}</div><div class='wl-meta'>\U0001f4c5 {T('added')}: {w.get('added_on','')}</div>", unsafe_allow_html=True)
                with c3:
                    if st.button(T("open"), key=f"wl_o_{i}"): goto("details", w["tmdb_id"])
                    if st.button("✕", key=f"wl_r_{i}"): remove_from_watchlist(w["tmdb_id"]); st.rerun()
...
# ==========================================================
    ...
    with st.spinner("Loading movies... (first load may take ~30s)"):
        home_cards, err = api_get_json("/home", params={"category": home_category, "limit": 24})
    if err or not home_cards:
        st.warning("Server is waking up. Please wait 30 seconds and refresh.")
        if st.button("Retry"):
            st.cache_data.clear()
            st.rerun()
        st.stop()
...
# ==========================================================
    ...
    with left:
        if data.get("poster_url"):
            ...
            st.markdown("<div style='background:#1a1a26;border-radius:12px;height:380px;display:flex;align-items:center;justify-content:center;color:#7a7590;font-size:3rem'>\U0001f3ac</div>", unsafe_allow_html=True)
    ...
    with right:
        ...
        if runtime:
            st.markdown(f"<div style='color:#7a7590;font-size:0.85rem;margin-top:8px'>\U0001f550 {runtime} {T('min')}</div>", unsafe_allow_html=True)
        st.markdown("---")
